...
swiper__WEBPACK_IMPORTED_MODULE_11__["Swiper"].use([swiper__WEBPACK_IMPORTED_MODULE_11__["Parallax"], swiper__WEBPACK_IMPORTED_MODULE_11__["Pagination"], swiper__WEBPACK_IMPORTED_MODULE_11__["Autoplay"]]);
window.addEventListener('DOMContentLoaded', function () {
  var changeProgress = function changeProgress(slider) {
    var progressBar = document.querySelector('.slider__progressbar');
    var i = slider.activeIndex + 1;
    var slides = slider.slides.length;
    var calc = i / slides * 100;
    progressBar.style.backgroundSize = "".concat(calc, "% 100%");
    progressBar.setAttribute('aria-valuenow', calc);
  };

  var swiper = new swiper__WEBPACK_IMPORTED_MODULE_11__["Swiper"]('.swiper-container', {
    loop: false,
    speed: 1500,
    spaceBetween: 150,
    parallax: true,
    ...
    pagination: {
      ...
      renderCustom: function renderCustom(swiper, current, total) {
        var currentRes = current < 10 ? "0".concat(current) : current;
        var totalRes = total < 10 ? "0".concat(total) : total;
        return "<span class=\"current-slide\">".concat(currentRes, "</span> / ").concat(totalRes);
      }
    },
    on: {
      init: function init() {
  ...
  var modal = function modal(modalSelector) {
    var modalEl = document.querySelector(modalSelector);
    var modalContent = modalEl.querySelector('.modal__dialog');
    var openTriggers = document.querySelectorAll('[data-modal-open]');
    var sendButton = modalEl.querySelector('button');
    var message = {
  ...
  var counter = function counter(selector, offset) {
    var counterEl = document.querySelector(selector);
    var elNumber = +counterEl.getAttribute('data-count');
    var isAnimated = false;

    var addDividerToNumber = function addDividerToNumber(number, divider) {
      return number.toFixed().toString().replace(/\B(?=(\d{3})+(?!\d))/g, divider);
    };

    var countTo = function countTo(el, number) {
      var end = number;
      var increment = number / 220;
      var i = 0;
      var timer = setInterval(function () {
        if (i >= end) {
          el.innerText = "".concat(addDividerToNumber(elNumber, '.'), "+");
          clearInterval(timer);
          return;
        }

        el.innerText = "".concat(addDividerToNumber(i += increment, '.'));
      }, 15);
    };
